...
public class c2_TransformingSequence extends TransformingSequenceBase {
    ...
    @Test
    public void maybe() {
        Mono<String> result = maybe_service().defaultIfEmpty(
                "no results"
        )
    ...
    public void sequence_sum() {
        ...
        Mono<Integer> sum = numerical_service().reduce(
                // Initial seed value
                0,
                // Accumulator function with arguments in the correct order
                Integer::sum
        );
    ...
    public void sequence_starts_with_zero() {
        Flux<Integer> result = Flux.just(0).concatWith(numerical_service());
